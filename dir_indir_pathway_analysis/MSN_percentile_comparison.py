...
if __name__ == '__main__':
    ...
    syn_prob = 0.6
    min_syn_size = 0.1
    f_name = "cajal/scratch/users/arother/bio_analysis_results/dir_indir_pathway_analysis/230612_j0251v5_MSN_percentile_comparison_mcl_%i_ax%i_synprob_%.2f" % (cl, cl_ax, syn_prob)
    ...
    log.fino(f'min comp len cells = {cl} µm, min comp len ax = {cl_ax} µm, syn prob = {syn_prob}, min synapse size = {min_syn_size}')
    ...
    filename_spiness_saving = analysis_params.file_locations
    ...
    for p in percentiles:
        MSN_id_dict[p] = load_pkl2obj("%s/full_MSN p%.2i_arr_c%i.pkl" % (filename_spiness_saving,p, cl))
        MSN_id_dict[100 - p] = load_pkl2obj("%s/full_MSN p%.2i_arr_c%i.pkl" % (filename_spiness_saving, 100 - p, cl))
    ...
    for p in percentiles:
        result_MSN_filename_p1 = axon_den_arborization_ct(ssd, celltype=2, percentile = p, filename=f_name,
                                                          full_cells=True, cellids = MSN_id_dict[p],
                                                          min_comp_len = cl, min_syn_size = min_syn_size)
        result_MSN_filename_p2 = axon_den_arborization_ct(ssd, celltype=2, percentile = 100 - p, filename=f_name,
                                                          full_cells=True, cellids = MSN_id_dict[100 - p],
                                                          min_comp_len = cl, min_syn_size = min_syn_size)
    ...
    for p in percentiles:
        MSN_connectivity_resultsfolder = synapses_between2cts(sd_synssv, celltype1=2, percentile_ct1 = p, filename=f_name,
                                                              full_cells=True, cellids1 = MSN_id_dict[p], cellids2 = MSN_id_dict[100 - p],
                                                              min_comp_len = cl, syn_prob_thresh = syn_prob, min_syn_size = min_syn_size)
    ...
    GPe_ids = analysis_params.load_full_cell_array(6)

    for p in percentiles:
        MSN_GPe_p1_connectivity_resultsfolder = synapses_between2cts(sd_synssv, celltype1=2, celltype2=6, percentile_ct1 = p,
                                                                     filename=f_name, full_cells=True, cellids1 = MSN_id_dict[p],
                                                                     cellids2 = GPe_ids, min_comp_len = cl,
                                                                     syn_prob_thresh = syn_prob, min_syn_size = min_syn_size)
        MSN_GPe_p2_connectivity_resultsfolder = synapses_between2cts(sd_synssv, celltype1=2, celltype2=6, percentile_ct1 = 100 - p,
                                                                     filename=f_name, full_cells=True, cellids1 = MSN_id_dict[100 - p],
                                                                     cellids2 = GPe_ids, min_comp_len = cl,
                                                                     syn_prob_thresh = syn_prob, min_syn_size = min_syn_size)
    ...
    GPi_ids = analysis_params.load_full_cell_array(7)
    for p in percentiles:
        MSN_GPi_p1_connectivity_resultsfolder = synapses_between2cts(sd_synssv, celltype1=2, celltype2=7, percentile_ct1 = p,
                                                                     filename=f_name, full_cells=True, cellids1 = MSN_id_dict[p],
                                                                     cellids2 = GPi_ids, min_comp_len = cl,
                                                                     syn_prob_thresh = syn_prob, min_syn_size = min_syn_size)
        MSN_GPi_p2_connectivity_resultsfolder = synapses_between2cts(sd_synssv, celltype1=2, celltype2=7, percentile_ct1 = 100 - p,
                                                                     filename=f_name, full_cells=True, cellids1 = MSN_id_dict[100- p],
                                                                     cellids2 = GPi_ids, min_comp_len = cl,
                                                                     syn_prob_thresh = syn_prob, min_syn_size = min_syn_size)
    ...
    STN_ids = analysis_params.load_full_cell_array(0)
    for p in percentiles:
        MSN_STN_p1_connectivity_resultsfolder = synapses_between2cts(sd_synssv, celltype1=2, celltype2=0, percentile_ct1 = p,
                                                                     filename=f_name, full_cells=True, cellids1 = MSN_id_dict[p],
                                                                     cellids2 = STN_ids, min_comp_len = cl,
                                                                     syn_prob_thresh = syn_prob, min_syn_size = min_syn_size)
        MSN_STN_p2_connectivity_resultsfolder = synapses_between2cts(sd_synssv, celltype1=2, celltype2=0, percentile_ct1 = 100 - p,
                                                                     filename=f_name, full_cells=True, cellids1 = MSN_id_dict[100 - p],
                                                                     cellids2 = STN_ids, min_comp_len = cl,
                                                                     syn_prob_thresh = syn_prob, min_syn_size = min_syn_size)
    ...
    FS_ids = analysis_params.load_full_cell_array(8)
    for p in percentiles:
        MSN_FS_p1_connectivity_resultsfolder = synapses_between2cts(sd_synssv, celltype1=2, celltype2=8, percentile_ct1 = p,
                                                                    filename=f_name, full_cells=True, cellids1 = MSN_id_dict[p],
                                                                    cellids2 = FS_ids, min_comp_len = cl,
                                                                    syn_prob_thresh = syn_prob, min_syn_size = min_syn_size)
        MSN_FS_p2_connectivity_resultsfolder = synapses_between2cts(sd_synssv, celltype1=2, celltype2=8, percentile_ct1 = 100 - p,
                                                                    filename=f_name, full_cells=True, cellids1 = MSN_id_dict[100 - p],
                                                                    cellids2 = FS_ids, min_comp_len = cl,
                                                                    syn_prob_thresh = syn_prob, min_syn_size = min_syn_size)
    ...
    TAN_ids = analysis_params.load_full_cell_array(5)
    for p in percentiles:
        MSN_TAN_p1_connectivity_resultsfolder = synapses_between2cts(sd_synssv, celltype1=2, celltype2=5, percentile_ct1 = p,
                                                                     filename=f_name, full_cells=True, cellids1 = MSN_id_dict[p],
                                                                     cellids2 = TAN_ids, min_comp_len = cl,
                                                                     syn_prob_thresh = syn_prob, min_syn_size = min_syn_size)
        MSN_TAN_p2_connectivity_resultsfolder = synapses_between2cts(sd_synssv, celltype1=2, celltype2=5, percentile_ct1 = 100 - p,
                                                                     filename=f_name, full_cells=True, cellids1 = MSN_id_dict[100 - p],
                                                                     cellids2 = TAN_ids, min_comp_len = cl,
                                                                     syn_prob_thresh = syn_prob, min_syn_size = min_syn_size)
    ...
    for p in percentiles:
        MSN_HVC_p1_connectivity_resultsfolder = synapses_ax2ct(sd_synssv, celltype1 = 4, filename = f_name, cellids1 = HVC_ids,
                                                               celltype2= 2, cellids2 = MSN_id_dict[p], full_cells_ct2= True, min_comp_len_cell = cl,
                                                               syn_prob_thresh = syn_prob, min_syn_size = min_syn_size, min_comp_len_ax = cl_ax,
        ...
        MSN_HVC_p2_connectivity_resultsfolder = synapses_ax2ct(sd_synssv, celltype1=4, celltype2=2,
                                                                     ...
                                                                     cellids2=MSN_id_dict[100 - p], min_comp_len_cell=cl,
                                                                     syn_prob_thresh=syn_prob, label_ct2 = "MSN p%.2i" % (100 - p),
                                                               min_syn_size = min_syn_size, min_comp_len_ax = cl_ax)
    ...
    for p in percentiles:
        MSN_LMAN_p1_connectivity_resultsfolder = synapses_ax2ct(sd_synssv, celltype1=3, filename=f_name,
                                                               ...
                                                               full_cells_ct2=True,
                                                               min_comp_len_cell=cl, syn_prob_thresh=syn_prob,
                                                               label_ct1=None, label_ct2="MSN p%.2i" % p,
                                                                min_syn_size = min_syn_size, min_comp_len_ax = cl_ax)
