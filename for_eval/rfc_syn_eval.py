...
if __name__ == '__main__':
    ...
    min_comp_len_cell = 50
    min_comp_len_ax = 200
    min_syn_size = 0.1
    # color keys: 'BlRdGy', 'MudGrays', 'BlGrTe','TePkBr', 'BlYw', 'STNGPINTv6', 'AxTePkBrv6', 'TePkBrNGF', 'TeBKv6MSNyw'
    f_name = f"cajal/scratch/users/arother/bio_analysis_results/for_eval/240724_j0251{version}_rfc_syn_eval_mcl_%i_ax%i_ms_%f" % (
    ...
    log.info('Step 1/X: Prepare evaluation table of all synapses without filtering, where one is axon')
    ...
    rndm_syn_table.loc[0: total_num_samples - 1, 'celltype 1'] = rndm_cts[:, 0]
    rndm_syn_table.loc[0: total_num_samples - 1, 'celltype 2'] = rndm_cts[:, 1]
    ...
    log.info('Step 2/X: Iterate over each celltypes check min length')
    suitable_ids_dict = {}
    all_suitable_ids = []
    all_cell_dict = {}
    all_celltypes = []
    ...
    log.info('Step 3/X: Filter synapses and select random ids')