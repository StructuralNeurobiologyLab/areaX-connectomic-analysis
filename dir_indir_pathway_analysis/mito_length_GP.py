if __name__ == '__main__':
    ...
    def mito_prox_dis(sso, compartment = 0, radius = 50):
        ...
        dist_list = [dist_d_array, dist_id_array, dist_pathlength, dist_vol, dist_num, dist_den, dist_bin_abs1, dist_bin_abs1_5, dist_bin_abs2, dist_bin_rel1, dist_bin_rel1_5, dist_bin_rel2]
        return prox_list, dist_list
    ...
    #                      FS=8, LTS=9, NGF=10
        ...
        start = time.time()
        f_name = ("u/arother/test_folder/210504_j0251v3_mitoGPcomp_hp")
        ...
        sum_prox = np.zeros((len(cellids), 1400))
        sum_dist = np.zeros((len(cellids), 4500))
        ...
        sum_prox_vol = np.zeros((len(cellids), 1400))
        sum_dist_vol = np.zeros((len(cellids), 4500))
        ...
        cellid_prox_arr = np.zeros(len(cellids))
        cellid_dist_arr = np.zeros(len(cellids))
        ...
        for i, cell in enumerate(tqdm(ssd.get_super_segmentation_object(cellids))):
            ...
            prox_mito_dict[cell.id] = {"diameter": prox_list[0], "id": prox_list[1], "pathlength":prox_list[2], "vol": prox_list[3],
                                       "density":prox_list[4], "index": prox_list[5],"bin_abs1": prox_list[6], "bin_abs1_5":prox_list[7],
                                       "bin_abs2":prox_list[8], "bin_rel1": prox_list[9], "bin_rel1_5": prox_list[10], "bin_rel2": prox_list[11]}
            dist_mito_dict[cell.id] = {"diameter": dist_list[0], "id": dist_list[1], "pathlength": dist_list[2],
                                       "vol": dist_list[3],"density": dist_list[4], "index": dist_list[5], "bin_abs1": dist_list[6],
                                       "bin_abs1_5": dist_list[7],"bin_abs2": dist_list[8], "bin_rel1": dist_list[9], "bin_rel1_5": dist_list[10],
                                       "bin_rel2": dist_list[11]}
            ...
            if len(np.nonzero(dist_ds)[0]) > 0:
                ...
                sum_dist_vol[i, 0:len(dist_ds)] = dist_vol
        ...
        # graphs per celltype
        if len(med_prox_arr > 0) == 0:
                raise ValueError
                return 0,0

        gpe_prox_inds = np.in1d(cellid_prox_arr, gpeids)
        gpe_dist_inds = np.in1d(cellid_dist_arr, gpeids)
        gpi_prox_inds = np.in1d(cellid_prox_arr, gpiids)
        gpi_dist_inds = np.in1d(cellid_dist_arr, gpiids)
        # gpe and gpi arrays
        #gpe

        gpemed_prox_arr = med_prox_arr[gpe_prox_inds]
        gpemean_prox_arr = mean_prox_arr[gpe_prox_inds]
        gpemed_dist_arr = med_dist_arr[gpe_dist_inds]
        gpemean_dist_arr = mean_dist_arr[gpe_dist_inds]
        gpesum_prox = sum_prox[gpe_prox_inds]
        if len(gpesum_prox) == 0:
            return 0, 0
        gpesum_dist = sum_dist[gpe_dist_inds]
        gpemed_prox_vol = med_prox_vol[gpe_prox_inds]
        gpemean_prox_vol = mean_prox_vol[gpe_prox_inds]
        gpemed_dist_vol = med_dist_vol[gpe_dist_inds]
        gpemean_dist_vol = mean_dist_vol[gpe_dist_inds]
        gpesum_prox_vol = sum_prox_vol[gpe_prox_inds]
        gpesum_dist_vol = sum_dist_vol[gpe_dist_inds]
        gpemean_prox_den_arr = mean_prox_den_arr[gpe_prox_inds]
        gpemed_prox_den_arr = med_prox_den_arr[gpe_prox_inds]
        gpemean_dist_den_arr = mean_dist_den_arr[gpe_dist_inds]
        gpemed_dist_den_arr = med_dist_den_arr[gpe_dist_inds]
        gpemean_prox_in_arr = mean_prox_in_arr[gpe_prox_inds]
        gpemed_prox_in_arr = med_prox_in_arr[gpe_prox_inds]
        gpemean_dist_in_arr = mean_dist_in_arr[gpe_dist_inds]
        gpemed_dist_in_arr = med_dist_in_arr[gpe_dist_inds]
        gpeprox_am = prox_am[gpe_prox_inds]
        gpedist_am = dist_am[gpe_dist_inds]

        gpemed_prox_arr = gpemed_prox_arr[gpemed_prox_arr > 0]
        gpemean_prox_arr = gpemean_prox_arr[gpemean_prox_arr > 0]
        gpemed_dist_arr = gpemed_dist_arr[gpemed_dist_arr > 0]
        gpemean_dist_arr = gpemean_dist_arr[gpemean_dist_arr > 0]
        gpesum_prox = np.hstack(gpesum_prox)
        gpesum_prox = gpesum_prox[gpesum_prox > 0]
        gpesum_dist = np.hstack(gpesum_dist)
        gpesum_dist = gpesum_dist[gpesum_dist > 0]
        gpemed_prox_vol = gpemed_prox_vol[gpemed_prox_vol > 0]
        gpemean_prox_vol = gpemean_prox_vol[gpemean_prox_vol > 0]
        gpemed_dist_vol = gpemed_dist_vol[gpemed_dist_vol > 0]
        gpemean_dist_vol = gpemean_dist_vol[gpemean_dist_vol > 0]
        gpesum_prox_vol = np.hstack(gpesum_prox_vol)
        gpesum_prox_vol = gpesum_prox_vol[gpesum_prox_vol > 0]
        gpesum_dist_vol = np.hstack(gpesum_dist_vol)
        gpesum_dist_vol = gpesum_dist_vol[gpesum_dist_vol > 0]
        gpemean_prox_den_arr = gpemean_prox_den_arr[gpemean_prox_den_arr > 0]
        gpemed_prox_den_arr = gpemed_prox_den_arr[gpemed_prox_den_arr > 0]
        gpemean_dist_den_arr = gpemean_dist_den_arr[gpemean_dist_den_arr > 0]
        gpemed_dist_den_arr = gpemed_dist_den_arr[gpemed_dist_den_arr > 0]
        gpemean_prox_in_arr = gpemean_prox_in_arr[gpemean_prox_in_arr > 0]
        gpemed_prox_in_arr = gpemed_prox_in_arr[gpemed_prox_in_arr > 0]
        gpemean_dist_in_arr = gpemean_dist_in_arr[gpemean_dist_in_arr > 0]
        gpemed_dist_in_arr = gpemed_dist_in_arr[gpemed_dist_in_arr > 0]
        gpeprox_am = gpeprox_am[gpeprox_am > 0]
        gpedist_am = gpedist_am[gpedist_am > 0]

        #gpi
        gpimed_prox_arr = med_prox_arr[gpi_prox_inds]
        gpimean_prox_arr = mean_prox_arr[gpi_prox_inds]
        gpimed_dist_arr = med_dist_arr[gpi_dist_inds]
        gpimean_dist_arr = mean_dist_arr[gpi_dist_inds]
        gpisum_prox = sum_prox[gpi_prox_inds]
        gpisum_dist = sum_dist[gpi_dist_inds]
        gpimed_prox_vol = med_prox_vol[gpi_prox_inds]
        gpimean_prox_vol = mean_prox_vol[gpi_prox_inds]
        gpimed_dist_vol = med_dist_vol[gpi_dist_inds]
        gpimean_dist_vol = mean_dist_vol[gpi_dist_inds]
        gpisum_prox_vol = sum_prox_vol[gpi_prox_inds]
        gpisum_dist_vol = sum_dist_vol[gpi_dist_inds]
        gpimean_prox_den_arr = mean_prox_den_arr[gpi_prox_inds]
        gpimed_prox_den_arr = med_prox_den_arr[gpi_prox_inds]
        gpimean_dist_den_arr = mean_dist_den_arr[gpi_dist_inds]
        gpimed_dist_den_arr = med_dist_den_arr[gpi_dist_inds]
        gpimean_prox_in_arr = mean_prox_in_arr[gpi_prox_inds]
        gpimed_prox_in_arr = med_prox_in_arr[gpi_prox_inds]
        gpimean_dist_in_arr = mean_dist_in_arr[gpi_dist_inds]
        gpimed_dist_in_arr = med_dist_in_arr[gpi_dist_inds]
        gpiprox_am = prox_am[gpi_prox_inds]
        gpidist_am = dist_am[gpi_dist_inds]

        gpimed_prox_arr = gpimed_prox_arr[gpimed_prox_arr > 0]
        gpimean_prox_arr = gpimean_prox_arr[gpimean_prox_arr > 0]
        gpimed_dist_arr = gpimed_dist_arr[gpimed_dist_arr > 0]
        gpimean_dist_arr = gpimean_dist_arr[gpimean_dist_arr > 0]
        gpisum_prox = np.hstack(gpisum_prox)
        gpisum_prox = gpisum_prox[gpisum_prox > 0]
        gpisum_dist = np.hstack(gpisum_dist)
        gpisum_dist = gpisum_dist[gpisum_dist > 0]
        gpimed_prox_vol = gpimed_prox_vol[gpimed_prox_vol > 0]
        gpimean_prox_vol = gpimean_prox_vol[gpimean_prox_vol > 0]
        gpimed_dist_vol = gpimed_dist_vol[gpimed_dist_vol > 0]
        gpimean_dist_vol = gpimean_dist_vol[gpimean_dist_vol > 0]
        gpisum_prox_vol = np.hstack(gpisum_prox_vol)
        gpisum_prox_vol = gpisum_prox_vol[gpisum_prox_vol > 0]
        gpisum_dist_vol = np.hstack(gpisum_dist_vol)
        gpisum_dist_vol = gpisum_dist_vol[gpisum_dist_vol > 0]
        gpimean_prox_den_arr = gpimean_prox_den_arr[gpimean_prox_den_arr > 0]
        gpimed_prox_den_arr = gpimed_prox_den_arr[gpimed_prox_den_arr > 0]
        gpimean_dist_den_arr = gpimean_dist_den_arr[gpimean_dist_den_arr > 0]
        gpimed_dist_den_arr = gpimed_dist_den_arr[gpimed_dist_den_arr > 0]
        gpimean_prox_in_arr = gpimean_prox_in_arr[gpimean_prox_in_arr > 0]
        gpimed_prox_in_arr = gpimed_prox_in_arr[gpimed_prox_in_arr > 0]
        gpimean_dist_in_arr = gpimean_dist_in_arr[gpimean_dist_in_arr > 0]
        gpimed_dist_in_arr = gpimed_dist_in_arr[gpimed_dist_in_arr > 0]
        gpiprox_am = gpiprox_am[gpiprox_am > 0]
        gpidist_am = gpidist_am[gpidist_am > 0]
        ...
        sns.distplot(med_dist_arr, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label="GP")
        sns.distplot(gpemed_dist_arr, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(gpimed_dist_arr, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        med_dist_filename = ('%s/median_dist_%s_%s.png' % (f_name, ct_dict[celltype], comp_dict[compartment]))
        plt.title('median mitochondrium diameter in distal %s in %.4s' % (comp_dict[compartment],ct_dict[celltype]))
        plt.xlabel('length in µm')
        plt.ylabel('count of cells')
        plt.legend()
        plt.savefig(med_dist_filename)
        plt.close()
        sns.distplot(mean_dist_arr, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label="GP")
        sns.distplot(gpemean_dist_arr, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(gpimean_dist_arr, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        ...
        plt.close()
        sns.distplot(med_prox_arr, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label = "GP")
        sns.distplot(gpemed_prox_arr, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(gpimed_prox_arr, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        med_prox_filename = ('%s/median_prox_%s_%s.png' % (f_name, ct_dict[celltype], comp_dict[compartment]))
        plt.title('median mitochondrium diameter in proximal %s in %.s' % (comp_dict[compartment], ct_dict[celltype]))
        plt.xlabel('length in µm')
        plt.ylabel('count of cells')
        plt.legend()
        plt.savefig(med_prox_filename)
        plt.close()
        sns.distplot(mean_prox_arr, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label="GP")
        sns.distplot(gpemean_prox_arr, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(gpimean_prox_arr, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        ...
        plt.close()
        sns.distplot(sum_prox, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label="GP")
        sns.distplot(gpesum_prox, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(gpisum_prox, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        ...
        plt.close()
        sns.distplot(sum_dist, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label="GP")
        sns.distplot(gpesum_dist, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(gpisum_dist, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        ...
        plt.close()
        #plots for volumen
        sns.distplot(med_dist_vol, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label = "GP")
        sns.distplot(gpemed_dist_vol, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(gpimed_dist_vol, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        ...
        plt.close()
        sns.distplot(mean_dist_vol, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label = "GP")
        sns.distplot(gpemean_dist_vol, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(gpimean_dist_vol, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        ...
        plt.close()
        sns.distplot(med_prox_vol, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label="GP")
        sns.distplot(gpemed_prox_vol, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(gpimed_prox_vol, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        ...
        plt.close()
        sns.distplot(mean_prox_vol, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label= "GP")
        sns.distplot(gpemean_prox_vol, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(gpimean_prox_vol, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        ...
        plt.close()
        sns.distplot(sum_prox_vol, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label= "GP")
        sns.distplot(gpesum_prox_vol, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(gpisum_prox_vol, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        ...
        plt.close()
        sns.distplot(sum_dist_vol, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label="GP")
        sns.distplot(gpesum_dist_vol, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(gpisum_dist_vol, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        sum_distv_filename = ('%s/sum_distv_%s_%s.png' % (f_name, ct_dict[celltype], comp_dict[compartment]))
        plt.title('sum of mitochondrium vol in distal %s in %.s' % (comp_dict[compartment], ct_dict[celltype]))
        plt.xlabel('vol in voxel')
        plt.ylabel('count of mitochondria')
        plt.legend()
        ...
        plt.close()
        #plots for density
        sns.distplot(mean_prox_den_arr, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label= "GP")
        sns.distplot(gpemean_prox_den_arr, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(gpimean_prox_den_arr, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        ...
        plt.close()
        sns.distplot(med_prox_den_arr, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label="GP")
        sns.distplot(gpemed_prox_den_arr, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(gpimed_prox_den_arr, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        ...
        plt.close()
        sns.distplot(mean_dist_den_arr, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label="GP")
        sns.distplot(gpemean_dist_den_arr, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(gpimean_dist_den_arr, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        ...
        plt.close()
        sns.distplot(med_dist_den_arr, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label="GP")
        sns.distplot(gpemed_dist_den_arr, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(gpimed_dist_den_arr, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        ...
        plt.close()
        sns.distplot(mean_prox_in_arr, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label="GP")
        sns.distplot(gpemean_prox_in_arr, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(gpimed_prox_in_arr, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        ...
        plt.close()
        sns.distplot(med_prox_in_arr, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label="GP")
        sns.distplot(gpemed_prox_in_arr, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(gpimed_prox_in_arr, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        ...
        plt.close()
        sns.distplot(mean_dist_in_arr, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label = "GP")
        sns.distplot(gpemean_dist_in_arr, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(gpimean_dist_in_arr, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        ...
        plt.close()
        sns.distplot(med_dist_in_arr, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label="GP")
        sns.distplot(gpemed_dist_in_arr, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(gpimed_dist_in_arr, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        ...
        plt.close()
        sns.distplot(prox_am, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label="GP")
        sns.distplot(gpeprox_am, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(gpiprox_am, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        med_disti_filename = ('%s/proxam_%s_%s.png' % (f_name, ct_dict[celltype], comp_dict[compartment]))
        plt.title('amount of mitochondria in proximal %s in %.s' % (comp_dict[compartment], ct_dict[celltype]))
        plt.xlabel('amount of mitochondria')
        plt.ylabel('count of cells')
        plt.legend()
        plt.savefig(med_disti_filename)
        plt.close()
        sns.distplot(dist_am, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label="GP")
        sns.distplot(gpedist_am, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(gpidist_am, hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        ...
        plt.close()
        #log plots
        sns.distplot(np.log10(med_dist_arr), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label="GP")
        sns.distplot(np.log10(gpemed_dist_arr), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(np.log10(gpimed_dist_arr), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        ...
        plt.savefig(med_dist_filename)
        plt.close()
        sns.distplot(np.log10(mean_dist_arr), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label="GP")
        sns.distplot(np.log10(gpemean_dist_arr), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(np.log10(gpimean_dist_arr), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        mean_dist_filename = ('%s/logmean_dist_%s_%s.png' % (f_name, ct_dict[celltype], comp_dict[compartment]))
        plt.title('log of mean mitochondrium diameter in distal %s in %.4s' % (comp_dict[compartment], ct_dict[celltype]))
        plt.xlabel('log of length in 10^ µm')
        plt.ylabel('count of cells')
        plt.legend()
        plt.savefig(mean_dist_filename)
        plt.close()
        sns.distplot(np.log10(med_prox_arr), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label= "GP")
        sns.distplot(np.log10(gpemed_prox_arr), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(np.log10(gpimed_prox_arr), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        ...
        plt.savefig(med_prox_filename)
        plt.close()
        sns.distplot(np.log10(mean_prox_arr), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label="GP")
        sns.distplot(np.log10(gpemean_prox_arr), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(np.log10(gpimean_prox_arr), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        ...
        plt.savefig(mean_prox_filename)
        plt.close()
        sns.distplot(np.log10(sum_prox), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label="GP")
        sns.distplot(np.log10(gpesum_prox), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(np.log10(gpisum_prox), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        ...
        plt.savefig(sum_prox_filename)
        plt.close()
        sns.distplot(np.log10(sum_dist), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label="GP")
        sns.distplot(np.log10(gpesum_dist), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(np.log10(gpisum_dist), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        ...
        plt.close()
        # plots for volumen
        sns.distplot(np.log10(med_dist_vol), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label="GP")
        sns.distplot(np.log10(gpemed_dist_vol), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(np.log10(gpimed_dist_vol), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        ...
        plt.xlabel('log of vol in 10^ voxel')
        plt.ylabel('count of cells')
        plt.legend()
        plt.savefig(med_distv_filename)
        plt.close()
        sns.distplot(np.log10(mean_dist_vol), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label="GP")
        sns.distplot(np.log10(gpemean_dist_vol), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(np.log10(gpimean_dist_vol), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        ...
        plt.savefig(mean_distv_filename)
        plt.close()
        sns.distplot(np.log10(med_prox_vol), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label="GP")
        sns.distplot(np.log10(gpemed_prox_vol), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(np.log10(gpimed_prox_vol), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        ...
        plt.savefig(med_proxv_filename)
        plt.close()
        sns.distplot(np.log10(mean_prox_vol), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label="GP")
        sns.distplot(np.log10(gpemean_prox_vol), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(np.log10(gpimean_prox_vol), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        ...
        plt.savefig(mean_proxv_filename)
        plt.close()
        sns.distplot(np.log10(sum_prox_vol), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label="GP")
        sns.distplot(np.log10(gpesum_prox_vol), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(np.log10(gpisum_prox_vol), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        ...
        plt.savefig(sum_proxv_filename)
        plt.close()
        sns.distplot(np.log10(sum_dist_vol), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label="GP")
        sns.distplot(np.log10(gpesum_dist_vol), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(np.log10(gpisum_dist_vol), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        ...
        plt.close()
        # plots for density
        sns.distplot(np.log10(mean_prox_den_arr), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label="GP")
        sns.distplot(np.log10(gpemean_prox_den_arr), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(np.log10(gpimean_prox_den_arr), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        ...
        plt.xlabel('log of amount of mitochondria per 10 µmin 10^ ')
        plt.ylabel('count of cells')
        plt.legend()
        plt.savefig(meanproxden_filename)
        plt.close()
        sns.distplot(np.log10(med_prox_den_arr), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label="GP")
        sns.distplot(np.log10(gpemed_prox_den_arr), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(np.log10(gpimed_prox_den_arr), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        ...
        plt.savefig(med_proxd_filename)
        plt.close()
        sns.distplot(np.log10(mean_dist_den_arr), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label="GP")
        sns.distplot(np.log10(gpemean_dist_den_arr), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(np.log10(gpimean_dist_den_arr), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        ...
        plt.savefig(mean_distd_filename)
        plt.close()
        sns.distplot(np.log10(med_dist_den_arr), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label="GP")
        sns.distplot(np.log10(gpemed_dist_den_arr), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(np.log10(gpimed_dist_den_arr), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        ...
        plt.savefig(med_distd_filename)
        plt.close()
        sns.distplot(np.log10(mean_prox_in_arr), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label="GP")
        sns.distplot(np.log10(gpemean_prox_in_arr), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(np.log10(gpimean_prox_in_arr), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        ...
        plt.savefig(mean_proxi_filename)
        plt.close()
        sns.distplot(np.log10(med_prox_in_arr), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label="GP")
        sns.distplot(np.log10(gpemed_prox_in_arr), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(np.log10(gpimed_prox_in_arr), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        ...
        plt.savefig(med_proxi_filename)
        plt.close()
        sns.distplot(np.log10(mean_dist_in_arr), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label="GP")
        sns.distplot(np.log10(gpemean_dist_in_arr), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(np.log10(gpimean_dist_in_arr), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        ...
        plt.savefig(mean_disti_filename)
        plt.close()
        sns.distplot(np.log10(med_dist_in_arr), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label="GP")
        sns.distplot(np.log10(gpemed_dist_in_arr), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(np.log10(gpimed_dist_in_arr), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        ...
        plt.savefig(med_disti_filename)
        plt.close()
        sns.distplot(np.log10(prox_am), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label="GP")
        sns.distplot(np.log10(gpeprox_am), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(np.log10(gpiprox_am), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        med_disti_filename = ('%s/logproxam_%s_%s.png' % (f_name, ct_dict[celltype], comp_dict[compartment]))
        plt.title('log of amount of mitochondria in proximal %s in %.s' % (comp_dict[compartment], ct_dict[celltype]))
        plt.xlabel('log of amount of mitochondria in 10^')
        plt.ylabel('count of cells')
        plt.legend()
        plt.savefig(med_disti_filename)
        plt.close()
        sns.distplot(np.log10(dist_am), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "black"}, kde=False, label="GP")
        sns.distplot(np.log10(gpedist_am), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "mediumorchid"}, kde=False, label="GPe")
        sns.distplot(np.log10(gpidist_am), hist_kws={"histtype": "step", "linewidth": 3, "alpha": 1, "color": "springgreen"}, kde=False, label="GPi")
        med_disti_filename = ('%s/logdistam_%s_%s.png' % (f_name, ct_dict[celltype], comp_dict[compartment]))
        plt.title('log of amount of mitochondria in distal %s in %.s' % (comp_dict[compartment], ct_dict[celltype]))
        plt.xlabel('log of amount of mitochondria in 10^ ')
        plt.ylabel('count of cells')
        plt.legend()
        plt.savefig(med_disti_filename)
