...
if __name__ == '__main__':
    ...
    from general.analysis_morph_helper import get_cell_length
    ...
    f_name = "cajal/scratch/users/arother/bio_analysis_results/dir_indir_pathway_analysis/231107_j0251v5_ax_fraglengths"
    ...
    for ct in ax_cts:
        ...
        all_ax_cts.append(np.zeros(len(ct_ids)) + ct_dict['ct'])
    ...
    if kruskal_res[1] < 0.05:
        ...
        for gc in group_comps:
            ...
            ranksum_res_df.loc[f'p-value', f'{axon_df[gc[0]]} vs {ax_groups_str[gc[1]]}'] = ranksum_res[1]
        ranksum_res_df.to_csv(f'{f_name}/ranksum_results.csv')

    log.info('Step 4/4: Plot results')
    sns.histplot(data = axon_df, x = 'skeleton length', hue = celltypes, fill=False,
                 kde=False, element='step')
    plt.ylabel('number of axons')
    plt.xlabel('skeleton length [µm]')
    plt.tile('Lengths of axon fragments')
